...
public class ReminderController {
	...
	public ResponseEntity<?> createReminder(@RequestBody Reminder reminder, HttpSession session) {
		if (session.getAttribute("loggedInUserId") != null && session != null) {
			if (reminderService.createReminder(reminder)) {
				return new ResponseEntity<Reminder>(reminder, HttpStatus.CREATED);
			} else {
				return new ResponseEntity<String>("Reminder already Exists", HttpStatus.CONFLICT);
			}
		} else {
	...
	public ResponseEntity<?> updateReminder(@RequestBody Reminder reminder, HttpSession session) {
		try {
			if (session != null && session.getAttribute("loggedInUserId") != null
					&& session.getAttribute("loggedInUserId").equals(reminder.getReminderCreatedBy())) {
	 ...
	 */
	@GetMapping("/reminder")
	public ResponseEntity<?> getAllReminder(HttpSession session) {
		if (session.getAttribute("loggedInUserId") != null && session != null) {
			List<Reminder> reminderList = reminderService.getAllReminderByUserId(session.getAttribute("loggedInUserId").toString());
	...
	public ResponseEntity<?> getReminder(@PathVariable int id, HttpSession session) {
		if (session != null && session.getAttribute("loggedInUserId") != null) {
			try {
